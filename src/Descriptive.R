...
h1 <- hist(fix2$DFC_CLA, xlab="Days from calving to resumption of cyclicity", ylim=c(0, 3050), main="Histogram of DFC_CLA")
text(h1$mids, h1$counts, labels=h1$counts, adj=c(0.5, -0.5))
summary(fix2$DFC_CLA)
which.max(fix2$DFC_CLA)
which.min(fix2$DFC_CLA)
h2 <- hist(fix2$DFCFirstHeat, xlab="Days from calving to first recognised heat alarm",xlim=c(0, 250), ylim=c(0,2000), main="Histogram of First Heat Alarm after Calving")
text(h2$mids, h2$counts, labels=h2$counts, adj=c(0.5, -0.5))
summary(fix2$DFCFirstHeat)
h3 <- hist(fix2$DaysToFirstIns, xlab="Days to first insemination", ylim=c(0, 3000))
text(h3$mids, h3$counts, labels=h3$counts, adj=c(0.5, -0.5))
summary(fix2$DaysToFirstIns)
