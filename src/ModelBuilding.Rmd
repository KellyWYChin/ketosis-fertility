...
```{r}
surv.heat <- survfit(Surv(DaysToCLA, FirstCLA) ~ 1, data = survival)
...
surv1.heat <- survfit(Surv(DaysToCLA, FirstCLA) ~ ad_Lactation, data=survival)
print(surv1.heat)
...
sd(survival$DaysToFirstHeat)
plot(survival$DaysToFirstHeat ~ survival$AvgKeto20_100)
boxplot(survival$DaysToFirstHeat ~ survival$FarmNo, xlab = "Farm ID", ylab = "Time from calving to first heat (day)")
ggplot(data = survival, aes(x = FarmNo, y = DaysToFirstHeat)) +
  geom_boxplot(outlier.color = "red") + 
  theme_classic() +
  labs(x = "Farm ID", y = "Days from calving to first heat (day)") +
  theme(axis.text.x = element_text(angle = 90))
...
anova(coxme6, coxme7) # model 6 is slightly better than model 7
...
me6.keto1 <- coxme(Surv(DaysToFirstHeat, FirstHeat)~ MaxKetoDIM60 + ad_Lactation + I(PeakDFC/7) + CalvingSeason + cut(CowNo, c(0, 100, 180, 1000)) + (1|FarmNo), data=survival)
res.keto1 <- resid(me6.keto1, type = "martingale")
...
# check MaxKetoDIM60 in coxme6
me6.keto2 <- coxme(Surv(DaysToFirstHeat, FirstHeat)~AvgKeto20_100 + ad_Lactation + I(PeakDFC/7) + CalvingSeason + cut(CowNo, c(0, 100, 180, 1000)) + (1|FarmNo), data=survival)
res.keto2 <- resid(me6.keto2, type = "martingale")
plot(survival$MaxKetoDIM60, res.keto2, main = "Residual of MaxKetoDIM60")
lines(lowess(survival$MaxKetoDIM60, res.keto2), col = "red")

# check ad_Lactation in coxme6
me6.lact <- coxme(Surv(DaysToFirstHeat, FirstHeat)~AvgKeto20_100 + MaxKetoDIM60 + I(PeakDFC/7) + CalvingSeason + cut(CowNo, c(0, 100, 180, 1000)) + (1|FarmNo), data=survival)
res.lact <- resid(me6.lact, type = "martingale")
plot(survival$ad_Lactation, res.lact, main = "Residual of ad_Lactation")
lines(lowess(survival$ad_Lactation, res.lact), col = "red")
```
