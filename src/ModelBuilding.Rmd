...
univ_models <- lapply(univ_formulas, function(x)(coxph(x, data = Three.surv)))
...
## First Heat
df.cox1.heat <- coxph(Surv(DFCFirstHeat, FirstHeat) ~ ad_Lactation + AvgKeto20_atleast10 + MaxKetoDFC20 + PeakDFC + AvgMY20 + CalvingSeason + CalvingYear + factor(FarmNo), data = Three.surv)
