...
```{r}
# DFCFirstHeat
ggplot(data=Three.surv, aes(x=DFCFirstHeat))+
  ...
  ylab("#cows")+
  theme_classic()
summary(Three.surv$DFCFirstHeat)
var(Three.surv$DFCFirstHeat)
sd(Three.surv$DFCFirstHeat)

plot(Three.surv$DFCFirstHeat ~ Three.surv$AvgKeto20_100)
boxplot(Three.surv$DFCFirstHeat ~ Three.surv$FarmNo)
boxplot(Three.surv$AvgKeto20_100 ~ Three.surv$FarmNo)
boxplot(Three.surv$DFCFirstHeat ~ Three.surv$ad_Lactation)
hist(Three.surv$)
```
...
cox.ketostatus
plot(survfit(cox.avgketo, conf.int = TRUE, data = Three.surv))

cox.avgketo <- coxph(Surv(DFCFirstHeat, FirstHeat)~ AvgKeto20_100, data = Three.surv)
cox.avgketo
...
```{r}
cox0 <- coxph(Surv(DFCFirstHeat, FirstHeat)~1, data=Three.surv)
cox10 <- coxph(Surv(DFCFirstHeat, FirstHeat)~AvgKeto20_100, data = Three.surv)

cox1 <- coxph(Surv(DFCFirstHeat, FirstHeat)~ AvgKeto20_100 + ad_Lactation + PeakDFC + AvgMY60 + CalvingSeason + cut(CowNo, c(0, 100, 180, 1000)), data=Three.surv)
cox1

coxme1 <- coxme(Surv(DFCFirstHeat, FirstHeat)~AvgKeto20_100 + ad_Lactation + PeakDFC + AvgMY60 + CalvingSeason + cut(CowNo, c(0, 100, 180, 1000)) + (1|FarmNo), data=Three.surv)
coxme1
anova(cox1, coxme1)
coxme2 <- coxme(Surv(DFCFirstHeat, FirstHeat)~AvgKeto20_100 + ad_Lactation + PeakDFC + CalvingSeason + cut(CowNo, c(0, 100, 180, 1000)) + (1|FarmNo), data=Three.surv)
coxme2
...
## Check Interaction ##
#######################
coxme3 <- coxme(Surv(DFCFirstHeat, FirstHeat)~AvgKeto20_100 + ad_Lactation + AvgKeto20_100:ad_Lactation + PeakDFC + CalvingSeason + cut(CowNo, c(0, 100, 180, 1000)) + (1|FarmNo), data=Three.surv)
coxme3
anova(coxme2, coxme3)
# don't need to consider interaction between ketosis and parity

coxme4 <- coxme(Surv(DFCFirstHeat, FirstHeat)~AvgKeto20_100 + ad_Lactation + AvgKeto20_100:PeakDFC + PeakDFC + CalvingSeason + cut(CowNo, c(0, 100, 180, 1000)) + (1|FarmNo), data=Three.surv)
coxme4
anova(coxme2, coxme4)
...
#par(mfrow = c(1,2))
plot(Three.surv$AvgKeto20_100, resid.cox0)
lines(lowess(Three.surv$AvgKeto20_100, resid.cox0))
plot(log(Three.surv$AvgKeto20_100), resid.cox0)
lines(lowess(log(Three.surv$AvgKeto20_100), resid.cox0))

plot(Three.surv$AvgKeto20_100, resid.cox10)
lines(lowess(Three.surv$AvgKeto20_100, resid.cox10))
plot(log(Three.surv$AvgKeto20_100), resid.cox10)
lines(lowess(log(Three.surv$AvgKeto20_100), resid.cox10))
```

# NO MISSING VALUES
```{r}
survival <- Three.surv %>%
  filter(!is.na(PeakDFC) &!is.na(AvgMY60))
summary(survival)
```
...
```{r}
coxme2.lact <- coxme(Surv(DFCFirstHeat, FirstHeat)~ AvgKeto20_100 + PeakDFC + CalvingSeason + cut(CowNo, c(0, 100, 180, 1000)) + (1|FarmNo), data=Three.surv)
res.coxme2.lact <- resid(coxme2.lact, type = "martingale")
plot(Three.surv$ad_Lactation, res.coxme2.lact, main = "Residual of ad_Lactation")
lines(lowess(Three.surv$ad_Lactation, res.coxme2.lact), col = "red")
```

# Df Beta residuals
```{r}
resid.coxme1 <- resid(coxme1, type = "dfbeta")
plot(Three.surv$AvgKeto20_100, resid.coxme1[,1], ylab = "dfbeta for AvgKeto20_100")
plot(Three.surv$ad_Lactation, resid.coxme1[,2], ylab="dfbeta for ad_Lactation")
resid.coxme1[,1]
```
