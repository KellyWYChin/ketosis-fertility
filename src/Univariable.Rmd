...
####################################################################################
##  TODO  ##
# Match the lactation with milk date!
df.merge0 <- df.merge %>%
  group_by(FarmNo, CowId, Lactation) %>%
  mutate(Match.lactation = as.numeric(difftime(Milkdate, CalvingDate, units = "days"))) %>%
  # Min.diff = the min. days between milk record and calving date
  mutate(Min.diff = min(Match.lactation),
         # Min.dfc = min. day of DFC
         Min.dfc = min(DFC)) %>%
  # Time difference between min. milking date and min DFC
  mutate(diff.dfc = Min.diff - Min.dfc) %>%
  arrange(FarmNo, CowId, DFC) #1702840 obs 15v

df.test <- df.merge0 %>%
  filter(diff.dfc > 150) %>%
  distinct(FarmNo, CalvingDate, .keep_all = TRUE) %>%
  arrange(FarmNo, CowId) #60 obs 15v
####################################################################################
...
df.merge1 <- df.merge1 %>%
  filter(DFC < 305) #1371880 obs 11v
...
df.merge2 <- df.merge1 %>%
  distinct(FarmNo, CowId) #5,058 cows
