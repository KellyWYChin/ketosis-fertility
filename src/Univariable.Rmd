...
df.ketosis20 <- merge(df.keto3, df.highketo.20d, by=c('FarmNo', 'CowId', 'Lactation'), all.x=TRUE, all.y=TRUE, sort = F) #5107 obs of 11 variables
...
df.onset20 <- all_keto_raw %>%
  ...
  filter(BHBraw == OnSet) %>%
  filter(rDFC == min(rDFC)) #5446 obs of 6 variables
...
#########################################
ketosis <- merge(df.ketosis20, df.ketosis60, by = c('FarmNo', 'CowId', 'Lactation'), all.x=TRUE, all.y=TRUE, sort = F) #5164 obs of 19 variables
rm(list = ls(pattern = "df"))
```
...
# Add calving year
Three <- Three %>%
  mutate(CalvingYear = format(CalvingDate, format = "%Y")) #4327 obs of 65v
```
```{r, warning=FALSE}
#################
#  Descriptive  #
...
Three <- Three %>%
  relocate(CowNo, .after = FarmNo) #4041 obs 66v
```
