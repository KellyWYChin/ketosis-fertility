...
df.insN$Last_PregDate <- as.Date(df.insN$Last_PregDate)
df.insN$Last_InsDate <- as.Date(df.insN$Last_InsDate)
df.insN$Gap <- difftime(df.insN$Last_AbortionDate, df.insN$Last_InsDate, units="days")
...
df.ins1$First_InsDate <- as.Date(df.ins1$First_InsDate)
df.ins1$First_PregDate <- as.Date(df.ins1$First_PregDate)
df.ins1$First_AbortionDate <- as.Date(df.ins1$First_AbortionDate)
df.ins1$Gap <- difftime(df.ins1$First_AbortionDate, df.ins1$First_InsDate, units="days")
...
# Time difference between (Preg and Calving Date) -> not important
...
# number of cows
df.cow <- df.preg1 %>%
  distinct(FarmNo, CowId, CalvingDate) #4,517 cows
...
# When Last_AI is 1, meaning the cow was inseminated only once
#df.data$Last_AI[df.data$Last_AI == 1] <- NA
