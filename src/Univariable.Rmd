...
# number of cows
df.cow <- df.preg1 %>%
  distinct(FarmNo, CowId, CalvingDate) #4,970 cows
...
df.ketosis60$nHighKeto20 <- ifelse((!is.na(df.ketosis60$MaxKeto20) & (df.ketosis60$MaxKeto20 < 0.08)), 0, df.ketosis60$nHighKeto20)
...
#  Descriptive  #
#################
# How many duplicated cows? -> 60 cows
...
df.two_parity <- df.two_parity[!is.na(df.two_parity$sample.calving), ] #116 obs
...
df.three$Last_AI <- as.factor(df.three$Last_AI)
ggplot(data=df.three, aes(x=factor(Last_AI))) +
